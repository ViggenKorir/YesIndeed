    ...
    <aside className="bg-white rounded shadow p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-2">{summary ?? 'No recommendations yet.'}</p>
        </div>
      ...
      {items && items.length > 0 ? (
              ...
              >
                <div className="flex-1">
                  <div className="flex items-baseline justify-between gap-4">
                    ...
                    <div className="text-right">
                      <div className="text-sm font-medium">{formatCurrency(it.total)}</div>
                      <div className="text-xs text-gray-500">({it.qty} × {formatCurrency(it.unitPrice)})</div>
                    </div>
                  </div>
                </div>
        ...
        <div className="mt-4 text-sm text-gray-500">No add-ons suggested for the selected options.</div>
      )}

      <div className="mt-4 text-xs text-gray-400">
        Recommendations are generated automatically — review and accept any add-ons you'd like included in the final quote.
      </div>
    </aside>
  );
}
